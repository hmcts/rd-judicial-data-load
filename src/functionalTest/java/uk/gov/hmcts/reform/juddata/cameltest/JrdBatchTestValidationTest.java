...
class JrdBatchTestValidationTest extends JrdBatchIntegrationSupport {
    ...
    void testParentOrchestrationJsrAuditTestAndPartialSuccess() throws Exception {
        ...
        Assertions.assertEquals("1", judicialUserProfileList.get(0).get("per_id"));
        Assertions.assertEquals("2", judicialUserProfileList.get(1).get("per_id"));
        Assertions.assertEquals("joe.bloggs@ejudiciary.net", judicialUserProfileList.get(0).get("ejudiciary_email"));
        Assertions.assertEquals("jo1e.bloggs@ejudiciary.net", judicialUserProfileList.get(1).get("ejudiciary_email"));
        Assertions.assertEquals(3, judicialUserProfileList.size());

        List<Map<String, Object>> judicialAppointmentList = jdbcTemplate.queryForList(appointmentSql);
        Assertions.assertNotNull(judicialAppointmentList.get(0).get("judicial_office_appointment_id"));
        Assertions.assertNotNull(judicialAppointmentList.get(1).get("judicial_office_appointment_id"));
        Assertions.assertEquals("1", judicialAppointmentList.get(0).get("per_id"));
        Assertions.assertEquals("2", judicialAppointmentList.get(1).get("per_id"));
        Assertions.assertEquals(2, judicialAppointmentList.size());
    ...
    void testParentOrchestrationJsrExceedsThresholdAuditTest() throws Exception {
        ...
        Assertions.assertTrue(errorDescription.contains("Jsr exception exceeds threshold limit"));
    }
    ...
    void testLeafFailuresInvalidJsr() throws Exception {
        ...
        Assertions.assertEquals("1", judicialBaseLocationType.get(1).get("base_location_id"));
        Assertions.assertEquals("2", judicialBaseLocationType.get(2).get("base_location_id"));
        Assertions.assertEquals("5", judicialBaseLocationType.get(3).get("base_location_id"));
        Assertions.assertEquals(6, judicialBaseLocationType.size());

        List<Map<String, Object>> judicialRegionType = jdbcTemplate.queryForList(regionSql);
        Assertions.assertEquals("1", judicialRegionType.get(1).get("region_id"));
        Assertions.assertEquals("4", judicialRegionType.get(2).get("region_id"));
        Assertions.assertEquals("5", judicialRegionType.get(3).get("region_id"));
        Assertions.assertEquals(4, judicialRegionType.size());

        List<Map<String, Object>> exceptionList = jdbcTemplate.queryForList(exceptionQuery);
        for (int count = 0; count < 4; count++) {
            Assertions.assertNotNull(exceptionList.get(count).get("table_name"));
            Assertions.assertNotNull(exceptionList.get(count).get("scheduler_start_time"));
            Assertions.assertNotNull(exceptionList.get(count).get("key"));
            Assertions.assertNotNull(exceptionList.get(count).get("field_in_error"));
            Assertions.assertNotNull(exceptionList.get(count).get("error_description"));
            Assertions.assertNotNull(exceptionList.get(count).get("updated_timestamp"));
        }
        Assertions.assertEquals(5, exceptionList.size());
    ...
    void testParentOrchestrationJsrSkipChildAppointmentRecordsForInvalidUserProfile() throws Exception {
        ...
        Assertions.assertEquals("1", judicialAppointmentList.get(0).get("per_id"));
        Assertions.assertEquals("2", judicialAppointmentList.get(1).get("per_id"));
        Assertions.assertEquals(3, judicialAppointmentList.size());

        List<Map<String, Object>> exceptionList = jdbcTemplate.queryForList(exceptionQuery);
        Assertions.assertEquals(4, exceptionList.size());
        Assertions.assertEquals("judicial-office-appointment", exceptionList.get(1).get("table_name"));
        Assertions.assertEquals("judicial-office-appointment", exceptionList.get(2).get("table_name"));
        Assertions.assertEquals(INVALID_JSR_PARENT_ROW, exceptionList.get(1).get("error_description"));
        Assertions.assertEquals(INVALID_JSR_PARENT_ROW, exceptionList.get(2).get("error_description"));

        Assertions.assertEquals(5L, exceptionList.get(1).get("row_id"));
        Assertions.assertEquals(6L, exceptionList.get(2).get("row_id"));
    }
    ...
    void testParentOrchestrationJsrSkipChildRecordsForeignKeyViolations() throws Exception {
        ...
        List<Map<String, Object>> judicialUserProfileList = jdbcTemplate.queryForList(userProfileSql);
        Assertions.assertEquals(7, judicialUserProfileList.size());

        List<Map<String, Object>> judicialAppointmentList = jdbcTemplate.queryForList(appointmentSql);
        Assertions.assertEquals(5, judicialAppointmentList.size());

        List<Map<String, Object>> exceptionList = jdbcTemplate.queryForList(exceptionQuery);
        Assertions.assertEquals(4, exceptionList.size());
        Assertions.assertEquals(MISSING_PER_ID, exceptionList.get(0).get("error_description"));
        Assertions.assertEquals("judicial-office-appointment", exceptionList.get(0).get("table_name"));
        Assertions.assertEquals(MISSING_LOCATION, exceptionList.get(1).get("error_description"));
        Assertions.assertEquals(MISSING_BASE_LOCATION, exceptionList.get(2).get("error_description"));
        Assertions.assertEquals(MISSING_PER_ID, exceptionList.get(3).get("error_description"));
        Assertions.assertEquals("judicial_office_authorisation", exceptionList.get(3).get("table_name"));
    }
    ...
    void testChildSkipsOnParentFailure() throws Exception {
        ...
        Assertions.assertEquals(3, auditList.size()); //Personal, Locations, base-locations only
    }
    ...
    private void validateLeafRoleJsr(List<Map<String, Object>> judicialUserRoleType) {
        Assertions.assertEquals(5, judicialUserRoleType.size());
        Assertions.assertEquals("1", judicialUserRoleType.get(1).get("role_id"));
        Assertions.assertEquals("3", judicialUserRoleType.get(2).get("role_id"));
        Assertions.assertEquals("7", judicialUserRoleType.get(3).get("role_id"));

        Assertions.assertEquals("Magistrate", judicialUserRoleType.get(1).get("role_desc_en"));
        Assertions.assertEquals("Advisory Committee Member - Non Magistrate",
                judicialUserRoleType.get(2).get("role_desc_en"));
        Assertions.assertEquals("MAGS - AC Admin User", judicialUserRoleType.get(3).get("role_desc_en"));
    }
